...
const SimpleNode: React.FC<NodeProps<NodeData>> = ({ id, data, selected }) => {
  ...
  return (
    <div className={`
      bg-white rounded-xl border-2 shadow-lg p-6 min-w-[280px] max-w-[360px] 
      transform transition-all duration-300 ease-in-out cursor-pointer
      hover:scale-105 hover:shadow-2xl hover:shadow-blue-500/20
      ${selected ? 'border-blue-500 shadow-xl shadow-blue-500/30' : 'border-gray-200'}
      group relative
      ${nodeType === 'ai' ? 'min-w-[360px] max-w-[400px]' : ''}
    `}>
      {/* Selection Indicator */}
      {selected && (
      ...
      {/* Content */}
      <div className="mb-4">
        <h3 className="font-bold text-gray-900 text-base mb-3 leading-tight group-hover:text-gray-700 transition-colors">
          {data.label}
        </h3>
      ...
      <button className={`
        ...
        ${nodeType === 'ai' ? 'animate-pulse' : ''}
      `}>
        <span>{nodeType === 'ai' ? '⚡' : '🚀'}</span>
        {nodeType === 'ai' ? 'Process with AI' : 'Open & Process'}
      </button>
...
const initialNodes: Node<NodeData>[] = [
  ...
  },
  {
    ...
    data: { 
      label: 'AI Assistant',
      type: 'ai',
      description: 'AI-powered assistant for intelligent processing and analysis.',
      content: 'Analyze this document and summarize key points',
      aiModel: 'GPT-4',
      aiStatus: 'completed',
      aiResponse: 'I can help you process and analyze various types of content. Connect nodes to me to get intelligent insights!'
    },
    dragHandle: '.drag-handle',
    selectable: true,
    draggable: true,
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-xl border border-white/50 px-8 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">AI-Enhanced Workflow Canvas</p>
            </div>
          </div>
        </div>
      </div>
